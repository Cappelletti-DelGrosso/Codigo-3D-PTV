...
function [curva, resultados]=correspon(trayectorias, dirparametros)
        ...
        if length(trayectorias{ii,1})>0 && length(trayectorias{kk,2})>0 
            ...
            if or(and(principio1<=principio2,principio2<=final1), and(principio1<=final2,final2<=final1))
                ...
                for jj = 1:(M-m+1)
                    cd ..
                    cd('Correspondencia')
                    [candidatos1, candidatos2]=correspondencia(trayectoria1(:,jj),trayectoria2(:,jj), 0, dirparametros, 0);
